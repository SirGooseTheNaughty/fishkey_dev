function meshGradientBg_init(params) {
  if (!params.meshgradURL) {
    return console.error("Не задана ссылка с meshgradient.com");
  }
  const meshgradArray = params.meshgradURL.split("/");
  const meshgradParams = meshgradArray[meshgradArray.length - 1];
  const canvasId = "mesh-canvas";
  const baseUrl = 'https://sirgoosethenaughty.github.io/fishkey_dev/modular/meshGradientBg';
  
  if (params.defaultImgUrl || params.defaultBgColor) {
  ...
  if (params.removeBgs && (params.defaultImgUrl || params.defaultBgColor)) {
  ...
  (Warp.prototype = new Object()),
    ...
    (Canvas.prototype.setup_programs = function () {
      (this.warp_program = this.program(
        "warp",
        "warp_vertex",
        "warp_fragment"
      )),
    ...
    (Canvas.prototype.update_position = function () {
      ...
      if (err > 0.001) {
        redraw();
        this.warp.src[this.movingIndex][0] += gain[0];
        this.warp.src[this.movingIndex][1] += gain[1];
        warps.update();
      }
    });
  let needsDraw = false;
  ...
  function init() {
    $('#allrecords').prepend(`<canvas id="mesh-canvas" width="1200" height="600" style="position: fixed; background-color: ${params.defaultBgColor || 'white'};"></canvas>`);
    ...
    const t = new URLSearchParams(meshgradParams),
    ...
    gradient.movingIndex = gradient.warp.src.length - 1;
    gradient.drag = [
      gradient.movingIndex,
